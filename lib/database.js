(function(){
	...
	*/
		...
		switch (type){
			...
			case "file" :
				...
				files.findOne({
					...
					if(!err){
						if(!object){
							this.addNewFileEntry(connection, item, user, data, res)
						}
						else{
	...
	/*
		...
		database document.
	*/
	module.exports.addNewFileEntry = function(connection, item, user, data, res){
			...
			{	
				...
				if(err){
					...
					if(result){
					  	...
					  	data.s3.getSignedUrl('putObject', data.s3Params, (err, data) => {
							if(err){
							  console.log(err);
							  res.status(200);
							  res.write("uploadErr");
		...
		games.updateOne({	
			...
			}, 
			{
				...
				if(err){
					...
					//We have to add the config file to the game's list of files
					fs.readFile('./library/tsconfig.json', (err, data) => {
						if(err){
							console.log(err);
							res.write("config error");
							res.status(200);
							return res.end();
						}
						else{
							console.log(data.toString());
							module.exports.addNewGameFile(connection, name, {gfile_name : 'tsconfig.json', gfile_contents : data.toString()}, user, res);
						}
					});
				}
			});		
